  ...
  const defaultColumns = [
    {
      ...
      minWidth: 100,
      headerName: 'ID',
      renderCell: ({ row }) => <Typography href={`/apps/invoice/preview/${row.id}`}>{`#${row.id}`}</Typography>
      // renderCell: ({ users }) => <Typography href={`/apps/invoice/preview/${users.id}`}>{`#${users.id}`}</Typography>
    },
    {
      flex: 0.1,
      minWidth: 80,
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        return row.balance !== 0 ? (
          <Typography sx={{ color: 'text.secondary' }}>{row.address.suite}</Typography>
        ) : (
          <CustomChip rounded size='small' skin='light' color='success' label='Paid' />
        )
      }
