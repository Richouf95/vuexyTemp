...
export const getStaticProps = async () => {
  ...
  const invoice = await fetch('http://localhost:4003/api/invoices')
  ...
  const defaultColumns = [
    {
      ...
      minWidth: 100,
      headerName: 'ID',
      renderCell: ({ row }) => <Typography href={`/apps/invoice/preview/${row._id}`}>{`#${row.id}`}</Typography>
    ...
    },
    {
      ...
      renderHeader: () => <Icon icon='tabler:trending-up' />,
      renderCell: ({ row }) => {
        const { client, username, email } = row
        ...
        return (
          ...
          >
            <CustomAvatar skin='light'  sx={{ width: '1.875rem', height: '1.875rem' }}>
              <Icon  />
            </CustomAvatar>
          </Tooltip>
        )
    ...
    },
    {
      ...
      field: 'total',
      headerName: 'Total',
      renderCell: ({ row }) => <Typography sx={{ color: 'text.secondary' }}>{`$${row.montantTotal || 0}`}</Typography>
    },
    {
      flex: 0.15,
      minWidth: 140,
      field: 'issuedDate',
      headerName: 'Issued Date',
      renderCell: ({ row }) => <Typography sx={{ color: 'text.secondary' }}>{row.client.tel}</Typography>
    },
    {
      flex: 0.1,
      minWidth: 100,
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Tooltip title='View'>
            <IconButton
              ...
              href={`/apps/invoice/preview/${row._id}`}
            >
